...
//        imgList.add("https://media.istockphoto.com/id/1267541412/ko/사진/빨간-산타-클로스-모자와-웃는-표정으로-크리스마스를-축하하는-행복한-강아지-개.jpg?s=2048x2048&w=is&k=20&c=iDp7DehrScAiZTNH-MnP05eOOaTpN3SVv-8DIH5dfXY=")
    ...
    private fun initIndicators() {
        ...
        for (indicator in indicators) {
            indicator.apply {
                setImageResource(R.drawable.ic_indicator_inactive) // 비활성화된 원 이미지
    ...
    private fun updateIndicators(position: Int) {
        ...
        for (i in 0 until childCount) {
            ...
            if (i == indicatorPosition) {
                imageView.setImageResource(R.drawable.ic_indicator_active) // 활성화된 원 이미지
            } else {
                imageView.setImageResource(R.drawable.ic_indicator_inactive) // 비활성화된 원 이미지
            }
        }
    }
}